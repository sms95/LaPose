...
def compute_degree_cm_mAP(final_results, synset_names, log_dir, degree_thresholds=[360], shift_thresholds=[100],
                          iou_3d_thresholds=[0.1], iou_pose_thres=0.1, use_matches_for_pose=False, eval_recon=False, plot_figure=False):
    ...
    if plot_figure:
        ...
        for cls_id in range(1, num_classes):
            class_name = synset_names[cls_id]
            ax_iou.plot(
                iou_thres_list[:-1], iou_3d_aps[cls_id, :-1], label=class_name, linewidth=4)
    ...
    if plot_figure:
        ...
        for cls_id in range(1, num_classes):
            ...
            print(class_name)
            ax_trans.plot(
                degree_thres_list[:-1], pose_aps[cls_id, :-1, -1], label=class_name, linewidth=4)

        ax_trans.plot(degree_thres_list[:-1], pose_aps[-1, :-1, -1], label='mean', linewidth=4)
        ...
        for cls_id in range(1, num_classes):
            class_name = synset_names[cls_id]
            print(class_name)
            ax_rot.plot([i/100 for i in shift_thres_list[:-1]],
                        pose_aps[cls_id, -1, :-1], label=class_name, linewidth=4)

        ax_rot.plot([i/100 for i in shift_thres_list[:-1]], pose_aps[-1, -1, :-1], label='mean', linewidth=4)
