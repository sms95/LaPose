...
def evaluate(argv):
    if FLAGS.coor_type == 'both':
        FLAGS.dinov2 = True
    if FLAGS.coor_type == 'dino':
        FLAGS.att_use_rgb = False
        FLAGS.dinov2 = True
    if FLAGS.pnp_att_mask:
        FLAGS.use_uncertainty_loss = True
        FLAGS.mask_attention_type = 'concat'
    ...
    if os.path.exists(pred_result_save_path) :#and not FLAGS.save_nocs_map:
        ...
        if FLAGS.use_scale_net:
            ...
            if FLAGS.dataset=='Real':
                ...
                if len(FLAGS.sn_path) != 0:
                    ...
                    resume_model_dict = torch.load("./pretrained/scale_net_Real.pth")
            else:
                print('Load CAMERA scale_net')
                if len(FLAGS.sn_path) != 0:
                    resume_model_dict = torch.load(FLAGS.sn_path)
                else:
                    resume_model_dict = torch.load("./pretrained/scale_net_CAMERA.pth")
            ...
            print('Warning: The scale related results are wrong')
        ...
        for i, data in tqdm(enumerate(val_dataloader, 1)):
            ...
            data = data_origin
            one_hot = [convert_to_one_hot(i, 6) for i in data['cat_id_0base']]
            ...
            with torch.no_grad():
                output_dict = network.forward(data, device, do_loss=False)
                ...
                if FLAGS.pre_norm:
                    pred_scale = scale_net(data, device)
                else:
                    pred_scale = torch.norm(output_dict['size'], p=2, dim=1)
                ...
                if FLAGS.pre_norm:
                    ...
                    pred_RT[:, :3, :3] = pred_RT[:, :3, :3] * pred_scale[:, None, None]
