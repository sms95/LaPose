...
class NocsDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for j in range(num_instance):
            ...
            if FLAGS.use_pnp != -1:
                coord_2d = get_2d_coord_np(im_W, im_H, norm=False).transpose(1, 2, 0) # whem use umeyama
            ...
            # prepare dino image
            roi_dino_img = crop_resize_by_warp_affine(
                image, bbox_center, img_scale, FLAGS.dino_img_size, interpolation=cv2.INTER_NEAREST
            )
            roi_dino_img = (roi_dino_img / 255.0 - self.img_mean) / self.img_std
            roi_dino_img = roi_dino_img.transpose(2, 0, 1)
            ...
            mean_shapes_CAMERA.append(mean_shape_CAMERA)
            ...
            roi_dino_imgs.append(roi_dino_img)
            gts_nocs_coor.append(roi_nocs_coord)

        full_img = cv2.resize(image, (256, 256))
        ...
        mean_shapes_CAMERA = np.array(mean_shapes_CAMERA)
